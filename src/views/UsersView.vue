...
const downloadUserTemplate = () => {
  ...
  const rows = 'Teacher Name,teacher@example.com,teacher,password123\nAdmin Name,admin@example.com,admin,password456\n';
  ...
  <div v-else>
    <div class="card">
      <div class="card-header" style="flex-wrap: wrap; gap: 1rem;">
        <h3 class="card-title" style="margin-bottom: 0;">User Management (Admins & Teachers)</h3>
        <div style="display: flex; gap: 0.5rem; align-items: center;">
          <!-- Template Download Link -->
          <button class="btn btn-secondary btn-sm" @click="downloadUserTemplate" title="Download CSV template">
            📄 Template
          </button>
          <!-- Import Button -->
          <button class="btn btn-secondary btn-sm" @click="triggerUserImport" title="Import users from CSV file">
            📥 Import
          </button>
        ...
        <div v-if="selectedIds.length > 0" style="background: var(--primary-light); padding: 0.75rem 1rem; border-radius: var(--radius-sm); border: 1px dashed var(--primary-color); display: flex; align-items: center; justify-content: space-between; margin: 1rem; margin-top: 0; gap: 1rem;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="btn btn-danger btn-sm" @click="bulkDelete" style="padding: 0.25rem 0.6rem; font-size: 0.75rem;">🗑️ Delete Selected</button>
          </div>
        </div>
        ...
        <table class="table">
          ...
          <tbody>
            <tr v-for="u in users" :key="u.id" :class="u.is_active === false ? 'row-inactive' : ''">
                ...
                <div class="action-group">
                  <div class="dropdown-wrapper" @click.stop>
                    <button
                      ...
                      title="More actions"
                    >⋮</button>
                    <div v-if="openDropdownId === u.id" class="dropdown-menu-custom">
                      <button
                        class="dropdown-item"
    ...
    <div v-if="showDetailsModal" class="modal-overlay">
      <div class="modal-content" style="max-width: 500px; width: 90%;">
        <div class="modal-header">
          <h3 class="modal-title">👁️ User Detailed Profile</h3>
        ...
        <div v-if="selectedUserDetails" style="padding: 1rem 0; text-align: center;">
          ...
          <div style="display: flex; gap: 0.5rem; justify-content: center; margin-bottom: 1rem;">
            <span class="badge" :class="selectedUserDetails.role === 'admin' ? 'badge-info' : 'badge-success'" style="font-size: 0.8rem; padding: 0.3rem 0.75rem;">
              Role: {{ selectedUserDetails.role.toUpperCase() }}
            </span>
            <span class="badge" :class="selectedUserDetails.is_active !== false ? 'badge-success' : 'badge-secondary'" style="font-size: 0.8rem; padding: 0.3rem 0.75rem;">
              Status: {{ selectedUserDetails.is_active !== false ? 'Active' : 'Inactive' }}
            </span>
          </div>
        </div>

        <div class="modal-footer" style="justify-content: center; padding-top: 0.5rem;">
          <button type="button" class="btn btn-secondary" @click="showDetailsModal = false" style="width: 100%;">Close Profile</button>
        </div>
      </div>
    </div>
  </div>
